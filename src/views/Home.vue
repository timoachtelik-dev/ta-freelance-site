        ...
        <div class="w-full max-w-xs sm:max-w-md">
          <div class="bg-gradient-to-br from-primary to-secondary rounded-2xl shadow-2xl p-2 sm:p-3 rotate-0 sm:rotate-1 lg:rotate-3 transition-all duration-300 hover:rotate-0 aspect-[16/10]">
            <div class="bg-background dark:bg-surfaceDark rounded-xl overflow-hidden p-3 sm:p-4 h-full flex flex-col">
              ...
              <pre class="mt-3 flex-1 overflow-hidden font-mono text-[10px] sm:text-xs leading-relaxed text-secondary dark:text-background" aria-hidden="true"><span class="text-accent">type</span> <span class="text-highlight dark:text-accent">Engineer</span> = {
  name: <span class="text-highlight dark:text-accent">string</span>;
  focus: <span class="text-highlight dark:text-accent">string</span>[];
};

<span class="text-accent">const</span> timo: <span class="text-highlight dark:text-accent">Engineer</span> = {
  name: <span class="text-primary dark:text-highlight">"Timo Achtelik"</span>,
  focus: [<span class="text-primary dark:text-highlight">"Angular"</span>, <span class="text-primary dark:text-highlight">"Vue"</span>, <span class="text-primary dark:text-highlight">"Node"</span>],
};<span class="inline-block w-1.5 h-3 sm:h-4 ml-0.5 align-middle bg-accent animate-pulse"></span></pre>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="mt-24">
      ...
      <div class="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div v-for="skill in orderedSkills" :key="skill.key"
          ...
          <p class="mt-2 text-sm text-primary dark:text-accent">
            {{ $t(`home.skills.items.${skill.key}.context`) }}
          </p>
        </div>
      </div>
    </div>

    <div v-if="projects.length" class="mt-24">
      <h2 class="text-2xl md:text-3xl font-bold text-secondary dark:text-background text-center">
      ...
      <div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-8">
        <article
          ...
          :key="project.key"
          class="flex flex-col bg-surface dark:bg-surfaceDark rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl"
        >
          <div class="h-2" :class="project.barClass"></div>
          <div class="p-6 flex flex-col flex-1">
            <span class="text-xs font-semibold uppercase tracking-wide text-accent">
              {{ $t(`home.projects.items.${project.key}.role`) }}
            </span>
            <h3 class="mt-1 text-lg font-semibold text-secondary dark:text-background">
              {{ $t(`home.projects.items.${project.key}.title`) }}
            </h3>
            <p class="mt-3 text-sm text-secondary/80 dark:text-background/80 flex-1">
              {{ $t(`home.projects.items.${project.key}.description`) }}
            </p>
            <p class="mt-4 text-xs font-mono text-primary dark:text-accent">
              {{ $t(`home.projects.items.${project.key}.stack`) }}
            </p>
            <a
              v-if="project.url"
              :href="project.url"
              ...
              class="mt-4 inline-flex items-center text-sm font-medium text-highlight hover:text-accent dark:text-accent"
            >
...
export default {
  computed: {
    ...
    orderedSkills() {
      // CV-core order first, then the rest.
      const order = ['angular', 'vue', 'typescript', 'node', 'postgres', 'docker', 'architecture', 'agile'];
      return [...this.skills].sort((a, b) => order.indexOf(a.key) - order.indexOf(b.key));
    },
  },
  data() {
    return {
